...
DEST_DIR = "/data/summer2020/naufal/final_embeddings_pca80"
...
tokens_reduced = tokens_reduced.reshape(-1, N_FEATURES_ORIG, N_TOKENS_PCA).transpose(0, 2, 1)  # (N, 80, 1541)

# === Fit PCA on feature axis (dimensionality reduction) ===
print("[✓] Fitting PCA to reduce feature dimension...")
flattened_feats = tokens_reduced.reshape(-1, N_FEATURES_ORIG)  # (N × 80, 1541)
...
for fname in tqdm(all_files, desc="Reducing and saving"):
    ...
    try:
        ...
        # Token PCA: transpose to (1541, 1913) → apply PCA → (1541, 80)
        token_input = x.T
        token_reduced = pca_tokens.transform(token_input).T  # (80, 1541)

        # Feature PCA: (80, 1541) → (80, 80)
        feat_reduced = pca_features.transform(token_reduced)  # (80, 80)
