...
class GnnPF(torch.nn.Module):
    ...
    def forward(self, esm_rep, seq, pssm, A, seq_embed, batch):
        ...
        out = torch.relu(out)
        out, A, _, batch, perm, score = self.gp1(out, A, None, batch)
        
        out = self.gc2(out, A)
        out = torch.relu(out)        
        out, A, _, batch, perm, score = self.gp2(out, A, None, batch)
        
        out = self.gc3(out, A)
        out = torch.relu(out)        
        out, A, _, batch, perm, score = self.gp3(out, A, None, batch)
      
        out = self.gc4(out, A)
        out = torch.relu(out)        
        out, A, _, batch, perm, score = self.gp4(out, A, None, batch)
